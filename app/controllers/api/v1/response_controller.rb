class ResponseController < ApplicationController
  ...
  def send_feedback_email
    feedback_response_map = FeedbackResponseMap.find_by(id: params[:id])

    unless feedback_response_map
        flash[:error] = 'Feedback response not found.'
        redirect_to some_path and return
    end

    begin
        visitor = EmailNotificationVisitor.new
        feedback_response_map.accept(visitor, defn, participant, assignment)
        flash[:success] = 'Feedback email notification sent successfully!'
    rescue StandardError => e
        flash[:error] = "Failed to send feedback email: #{e.message}"
        Rails.logger.error "Feedback email error: #{e.message}"
    end

    redirect_to send_feedback_email_api_v1_response_index_path
  end
